...
burger.addEventListener('click', () => {
    ...
    if(!burger.classList.contains('cross')) {
        ...
        navLinks.forEach((link, index) => {
            link.style.animation = `entry ${(index * .2)+.5}s .3s forwards`
        })
    } else {
        navLinks.forEach((link, index) => {
            link.style.animation = `exit ${(index * .2)+.5}s forwards`
        })
        setTimeout(()=> {
...
inputs.forEach((input,index) => {
    input.addEventListener("focus", (e) => {
        ...
        if(!focus) {
            ...
            if(inputs[0].classList.contains("valid-input") &&
